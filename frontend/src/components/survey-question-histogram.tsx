...
interface SurveyQuestionHistogramProps {
  title: ReactNode;
  tooltip: ReactNode;
  surveyQuestion: components['schemas']['SurveyResponseRecourse'];
}
    ...
    <Paper className='h-full w-[525px] pb-8 pr-8'>
      <div className='flex justify-between pb-6'>
        ...
        <div className='flex items-start gap-2'>
          ...
          <h3 className='text-md pt-1 text-foreground sm:text-lg md:text-xl'>
            / 5
          </h3>
        </div>
      </div>
      ...
      <div
        ...
        `}
      >
        {Object.entries(surveyQuestion.options).map(([label, response]) => (
          <SurveyQuestionHistogramBar
            key={response}
            label={response}
            value={surveyQuestion[label as 'A' | 'B' | 'C' | 'D' | 'E' | 'F']}
